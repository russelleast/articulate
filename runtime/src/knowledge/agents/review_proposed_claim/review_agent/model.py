...
class DaprPromptyClaimReviewer:
    def __init__(
        self, prompty_path: Path, component_name: str, maximum_concurrency: int = 4
    ) -> None:
        ...
        self._capacity = asyncio.Semaphore(maximum_concurrency)
    ...
    async def review(self, claim: ClaimInput) -> ModelReview:
        with trace.get_tracer("articulate.review_proposed_claim").start_as_current_span(
            ...
            async with self._capacity:
                # dapr-agents 1.0.5 exposes only a blocking generate API.
