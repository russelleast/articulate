...
const subtitlePath = path.resolve(repoRoot, "production/episodes/0014/output/episode-0014-final.srt");
const thumbnailPath = path.resolve(repoRoot, "production/episodes/0014/publication/thumbnail/episode-0014-thumbnail.png");
...
const report = { version: 1, status: errors.length ? "failed" : "passed", source: "production/episodes/0014/Ep14-narrator.mp4", output: config.output.video, subtitle: "production/episodes/0014/output/episode-0014-final.srt", thumbnail: "production/episodes/0014/publication/thumbnail/episode-0014-thumbnail.png", selectedDurationSeconds: config.presenter.expectedDurationSeconds, frameRate: 30, sceneCount: config.scenes.length, continuousPresenter: true, diagramAssets: diagramIds, checks: ["continuous presenter source", "complete recording coverage", "1080p 30 fps publication output", "new information remains proposed before acceptance", "Once Was, As Is and To Be remain connected", "approval is separated from acceptance", "claims assemble into architectural relationships", "subtitle timing and typography limits", "series-consistent thumbnail"], errors };
fs.writeFileSync(path.resolve(repoRoot, "production/episodes/0014/output/review/presenter-v1/episode-validation.json"), `${JSON.stringify(report, null, 2)}\n`);
if (errors.length) { console.error(errors.join("\n")); process.exit(1); }
console.log("Episode 0014 presenter validation passed.");
