...
const episodeDirectory = path.join(repoRoot, "production/episodes/0014");
...
const rawPath = path.join(here, "raw-transcription/episode-0014-presenter-whisper-base-en.json");
...
const config = {
  ...
  episode: {
    ...
    storyboard: "production/episodes/0014/storyboard.yaml",
    timingMarkers: "production/episodes/0014/production/presenter-v1-markers.json",
    assetRegister: "production/episodes/0014/asset-register.yaml",
    scenePlan: "production/episodes/0014/scene-plan.yaml",
    timeline: "production/episodes/0014/timeline.json"
  },
  presenter: {
    implementation: "continuous-video",
    assetId: "episode-0014-presenter-v1",
    ...
    background: "#000000",
    compositing: { mode: "soft-luma-key", applyTo: ["presenter-focus"], darkThreshold: 0.025, softThresholdRange: 0.055, maskFeatherRadius: 2, minimumRetainedOpacity: 0, maskContrast: 1, edgeVignetteRadius: 72, darkHaloSuppression: 0.03, temporalSmoothing: 2 },
  ...
  output: { width: 1920, height: 1080, frameRate: 30, video: "production/episodes/0014/output/episode-0014-final.mp4", generatedDirectory: "production/episodes/0014/generated/presenter-v1", reviewDirectory: "production/episodes/0014/output/review/presenter-v1", narrationAnalysis: "production/episodes/0014/production/presenter-v1-analysis.json" },
  scenes
};

const storyboard = {
  version: 2,
  episode: { id: "0014", title: plan.episode.title, written_source: plan.episode.writtenSource, narrative_source: plan.episode.narrativeSource, scene_plan: "production/episodes/0014/scene-plan.yaml", production_status: "presenter-v1" },
...
const timeline = { version: 2, generatedFrom: "production/episodes/0014/scene-plan.yaml", authority: "recorded-presenter", selectedDurationSeconds: duration, scenes: markers.scenes.map((marker, index) => ({ ...marker, archetype: plan.scenes[index].archetype, kind: plan.scenes[index].kind, transition: plan.scenes[index].transition })) };
const sceneList = { episode: { id: "episode-0014", title: plan.episode.title, production_status: "presenter-v1", timing_authority: "episode-0014-presenter-v1", narration_duration_seconds: duration }, scenes: storyboard.scenes.map((scene) => ({ scene_id: scene.id, start_seconds: scene.start_seconds, end_seconds: scene.end_seconds, visual_type: scene.archetype, presenter_mode: scene.kind === "presenter-focus" ? "visible" : "audio-continuous", diagram_asset: scenes.find((item) => item.id === scene.id)?.diagramAssetId ?? null })) };
...
console.log(`Episode 0014 publication duration: ${duration.toFixed(3)} seconds`);

function correctTranscript(source) {
  const replacements = [
    [/\bAI native\b/gi, "AI-native"], [/\bPostgres SQL\b/gi, "PostgreSQL"],
    [/\bPostgres\b/gi, "PostgreSQL"], [/\bMongo\b/gi, "MongoDB"], [/\bDynamo\b/gi, "DynamoDB"],
    [/\bwithout articulating\b/gi, "Within Articulate"], [/\bto articulate to understand\b/gi, "Articulate to understand"],
    [/\bscheduled for assignment\b/gi, "scheduled for retirement"],
...
function writeSubtitles(source, mediaDuration) {
  ...
  fs.writeFileSync(path.join(publication, "episode-0014-en.srt"), srt);
  fs.writeFileSync(path.join(publication, "episode-0014-transcript.txt"), `${plain}\n`);
  fs.writeFileSync(path.join(episodeDirectory, "output/episode-0014-final.srt"), srt);
  fs.writeFileSync(path.join(publication, "episode-0014-subtitle-validation.md"), "# Episode 0014 Subtitle Validation\n\n- Source: final continuous presenter recording\n- Timing: local Whisper word timestamps corrected against the recorded delivery\n- Language: British English\n- Maximum lines: two\n- Maximum line length: 42 characters\n- Terminology checked: Articulate, AI-native, Knowledge Model, PostgreSQL, MongoDB, DynamoDB, AWS ECS and AWS RDS\n- Coverage: complete presenter recording, including the final sign-off\n");
}
...
function subtitleWords(source) {
  ...
  const phrases = [
    ...
    [["postgressql"], ["PostgreSQL"]], [["postgres", "sql"], ["PostgreSQL"]], [["postgres"], ["PostgreSQL"]],
    [["mongo", "db"], ["MongoDB"]], [["mongo"], ["MongoDB"]], [["dynamo", "db"], ["DynamoDB"]], [["dynamo"], ["DynamoDB"]],
    [["aws", "ecs"], ["AWS", "ECS"]], [["aws", "rds"], ["AWS", "RDS"]], [["articulates"], ["Articulate"]]
  ];
