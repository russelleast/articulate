...
const framePath = path.join(sourceDirectory, "episode-0007-presenter-646_000.png");
const blendedPath = path.join(sourceDirectory, "episode-0007-presenter-646_000-blended.png");
const editablePath = path.join(sourceDirectory, "episode-0007-thumbnail.svg");
const outputPath = path.join(here, "episode-0007-thumbnail.png");
...
await sharp(outputPath).resize(320, 180).png().toFile(path.join(previewDirectory, "episode-0007-thumbnail-320.png"));
await sharp(outputPath).resize(160, 90).png().toFile(path.join(previewDirectory, "episode-0007-thumbnail-160.png"));
const data = `data:image/png;base64,${(await fs.readFile(outputPath)).toString("base64")}`;
await sharp(Buffer.from(contactSheet(data)), { density: 144 }).resize(1600, 820).png().toFile(path.join(here, "episode-0007-thumbnail-contact-sheet.png"));
console.log(`Episode 0007 thumbnail: ${path.relative(repoRoot, outputPath)}`);
...
function foregroundSvg() {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720"><defs><marker id="arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M0 0L10 5L0 10Z" fill="#6f93a4"/></marker></defs><g transform="translate(64 75)"><text font-family="Inter,Arial,sans-serif" font-size="18" font-weight="750" fill="#f4f0e8" letter-spacing="5.2">ARTICULATE</text><text y="23" font-family="Inter,Arial,sans-serif" font-size="9" font-weight="650" fill="#6f95a5" letter-spacing="2.1">AI-NATIVE ARCHITECTURE JOURNAL</text></g><g transform="translate(1140 60)"><rect width="80" height="34" rx="17" fill="#172a34" stroke="#55798b"/><text x="40" y="22" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="12" font-weight="750" fill="#f4f0e8" letter-spacing="1.4">EP 7</text></g><path d="M514 147H1190" stroke="#55798b" opacity=".42"/><g transform="translate(566 170)"><rect y="-19" width="96" height="6" rx="3" fill="#a65d3f"/><text y="34" font-family="Inter,Arial,sans-serif" font-size="14" font-weight="750" letter-spacing="2.4" fill="#8fa2aa">${config.episode.eyebrow}</text><text y="105" font-family="Inter,Arial,sans-serif" font-size="52" font-weight="800" letter-spacing="-1.8" fill="#f4f0e8">${config.episode.headline[0]}</text><text y="173" font-family="Inter,Arial,sans-serif" font-size="56" font-weight="800" letter-spacing="-1.9" fill="#c77a58">${config.episode.headline[1]}</text></g><g transform="translate(700 455) scale(.78)" font-family="Inter,Arial,sans-serif"><rect x="190" y="70" width="250" height="88" rx="18" fill="#18313a" stroke="#a65d3f" stroke-width="2"/><text x="315" y="105" text-anchor="middle" font-size="13" font-weight="800" fill="#f4f0e8" letter-spacing="1.5">KNOWLEDGE MODEL</text><text x="315" y="130" text-anchor="middle" font-size="11" fill="#9fc1cf">SHARED UNDERSTANDING</text><g font-size="11" font-weight="700" fill="#dbe8eb"><rect x="0" y="0" width="142" height="46" rx="10" fill="#142832" stroke="#55798b"/><text x="71" y="28" text-anchor="middle">STRUCTURE</text><rect x="0" y="90" width="142" height="46" rx="10" fill="#142832" stroke="#55798b"/><text x="71" y="118" text-anchor="middle">PRINCIPLES</text><rect x="490" y="0" width="142" height="46" rx="10" fill="#142832" stroke="#55798b"/><text x="561" y="28" text-anchor="middle">DECISIONS</text><rect x="490" y="90" width="142" height="46" rx="10" fill="#142832" stroke="#55798b"/><text x="561" y="118" text-anchor="middle">CLAIMS</text></g><path d="M142 23C205 23 194 85 190 105M142 113H190M490 23C430 23 446 85 440 105M490 113H440" fill="none" stroke="#6f93a4" stroke-width="2" marker-end="url(#arrow)"/></g></svg>`;
}

function contactSheet(imageHref) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="820"><rect width="1600" height="820" fill="#081219"/><text x="48" y="55" font-family="Inter,Arial,sans-serif" font-size="27" font-weight="800" fill="#f4f0e8">EPISODE 0007 · THUMBNAIL REVIEW</text><text x="48" y="88" font-family="Inter,Arial,sans-serif" font-size="15" fill="#8fa2aa">Full composition · 25% scale · mobile scale</text><image href="${imageHref}" x="48" y="150" width="960" height="540"/><image href="${imageHref}" x="1060" y="150" width="320" height="180"/><image href="${imageHref}" x="1060" y="408" width="160" height="90"/><text x="48" y="742" font-family="Inter,Arial,sans-serif" font-size="14" fill="#8fa2aa">Presenter left · Knowledge Model diagram right · minimal title treatment</text></svg>`;
}
