...
const markers = readJson(path.join(scriptDir, "narration-markers.json"));
const masterAudio = path.join(repoRoot, "production/narrator/Articulate-Ep1.wav");
const cacheDir = path.join(repoRoot, "production/cache/episode-0001/rough-cut-03-companion");
const timelineDir = path.join(scriptDir, "rough-cut-03-companion");
...
for (const marker of markers.scenes.filter((scene) => companionSceneIds.has(scene.id))) {
  ...
  run("node", [
    ...
    "--seed", String(2000 + Number(marker.id.slice(1))), "--motion-only"
  ]);
}

console.log(`Episode 0001 Rough Cut 03 Companion timelines: ${relative(timelineDir)}`);
