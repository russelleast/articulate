...
const episodeDirectory = path.join(repoRoot, "production/episodes/0002");
const planPath = path.join(episodeDirectory, "scene-plan.yaml");
const plan = readJson(planPath);
const mediaPath = path.join(repoRoot, plan.episode.audio);
const rawPath = path.join(here, "raw-transcription/episode-0002-presenter-whisper-base-en.json");
const selectedDurationSeconds = 358.6;
const transcript = normaliseWhisperTranscript(readJson(rawPath), {
  audio: plan.episode.audio,
  model: "whisper.cpp base.en",
  corrections: {
    "Hello. In the previous episode, I talked about why I started Articulate. The problem": "Hello. In the previous episode, I talked about why I started Articulate. The problem",
    "of trying to understand an architecture in the knowledge is scattered across documents,": "of trying to understand an architecture when the knowledge is scattered across documents,",
    "platform. Everyone of those descriptions captured something I thought was important, but none": "platform. Every one of those descriptions captured something I thought was important, but none",
    "architectural tools are built around artifacts, documents, diagrams, decision records, source": "architecture tools are built around artefacts: documents, diagrams, decision records, source",
    "emerges because something failed. Bit by bit are understanding of the architecture changes.": "emerges because something failed. Bit by bit, our understanding of the architecture changes.",
    "That's what Articulate architecture is really about. Not a collection of documents. A body": "That's what architecture is really about. Not a collection of documents. A body",
    "the wrong thing as the primary asset. Maybe the knowledge should come first. And the documents": "the wrong thing as the primary asset. Maybe the knowledge should come first, and the documents",
    "to diagrams, reports just become different views of that knowledge. That single idea": "and diagrams and reports just become different views of that knowledge. That single idea",
    "Changing assumptions. Find information that would have taken me hours to discover manually.": "Challenge assumptions. Find information that would have taken me hours to discover manually.",
    "for me. Architecture still requires judgment, experience, trade-offs, and accountability.": "for me. Architecture still requires judgement, experience, trade-offs and accountability.",
    "me think clearly, helping me discover more quickly. Helping me see things I might have": "me think more clearly, helping me discover more quickly, helping me see things I might have",
    "why was this design this way? What depends on this service? Have we solved this problem": "why was this designed this way? What depends on this service? Have we solved this problem",
    "to understand the systems they're responsible for. The phrase I gradually started using": "to understand the systems they're responsible for. The phrase I've gradually started using",
    "that's the future I want to explore. Not replacing architects, but given architects a better way": "that's the future I want to explore. Not replacing architects, but giving architects a better way",
    "for this is architectural intelligence, not artificial intelligence. The ability to preserve": "for this is Architectural Intelligence. Not Artificial Intelligence. The ability to preserve",
    "architectural knowledge, understanding relationships and reason about change. Explain decisions": "architectural knowledge, understand relationships and reason about change, explain decisions",
    "A.I. native platform designed to help architects understand, reason about and evolve complex systems.": "An AI-native platform designed to help architects understand, reason about and evolve complex systems.",
    "In the next episode, we'll build on that vision and by asking another important question. What": "In the next episode, we'll build on that vision by asking another important question. What",
    "does it actually mean to build an A.I. native system?": "does it actually mean to build an AI-native system?"
  }
});
...
const scenes = plan.scenes.map((scene) => {
  ...
  return {
    ...
    items: scene.items ?? [],
    companion: false,
    assetIds: scene.kind === "presenter-focus" ? ["A001", "A002"] : ["A002"],
...
const config = {
  ...
  episode: {
    ...
    storyboard: "production/episodes/0002/storyboard-presenter.yaml",
    timingMarkers: "production/episodes/0002/production/presenter-v1-markers.json",
    assetRegister: "production/episodes/0002/asset-register.yaml",
    scenePlan: "production/episodes/0002/scene-plan.yaml"
  },
  presenter: {
    implementation: "continuous-video",
    assetId: "episode-0002-presenter-v1",
    ...
    safeCrop: {
      ...
      note: "Episode 0001 presenter framing: complete source scaled to 780x439 and anchored lower-left."
    }
  },
  ...
  output: {
    ...
    generatedDirectory: "production/episodes/0002/generated/presenter-v1",
    reviewDirectory: "production/episodes/0002/output/review/presenter-v1",
    narrationAnalysis: "production/episodes/0002/production/presenter-v1-analysis.json"
  },
  scenes
};
...
const storyboard = {
  ...
  episode: {
    ...
    scene_plan: "production/episodes/0002/scene-plan.yaml",
...
const sceneList = {
  episode: {
    ...
    title: plan.episode.title,
    production_status: "presenter-v1",
    timing_authority: "episode-0002-presenter-v1",
  ...
  scenes: storyboard.scenes.map((scene) => ({
    ...
    presenter_mode: scene.kind === "presenter-focus" ? "visible-soft-luma" : "audio-continuous",
...
console.log(`Episode 0002 render config: ${relative(path.join(here, "presenter-v1-config.json"))}`);
console.log("Episode 0002 publication duration: 00:05:58.600");
...
function writePublicationSubtitles(source, duration) {
  ...
  if (groups.length > 1 && groups.at(-1).length < 3) {
    groups.at(-1).unshift(...groups.at(-2).splice(-Math.min(3, groups.at(-2).length - 3)));
  }
  ...
  fs.writeFileSync(path.join(subtitleDirectory, "episode-0002-en.srt"), srt);
  const outputSubtitle = path.join(episodeDirectory, "output/episode-0002-final.srt");
  fs.mkdirSync(path.dirname(outputSubtitle), { recursive: true });
  fs.writeFileSync(outputSubtitle, srt);
  fs.writeFileSync(path.join(subtitleDirectory, "episode-0002-transcript.txt"), `${transcriptText}\n`);
}
