...
const masterAudio = path.join(repoRoot, "production/episodes/0002/ep2-what-is-articulate.wav");
const narrativePath = path.join(repoRoot, "production/episodes/0002/narrative.md");
const cacheDir = path.join(repoRoot, "production/cache/episode-0002/rough-cut-01-companion");
const timelineDir = path.join(scriptDir, "rough-cut-01-companion");
const subtitleDir = path.join(repoRoot, "production/episodes/0002/publication/subtitles");
const companionSceneIds = new Set(["S001", "S005", "S008", "S013", "S015", "S017"]);
...
const sections = [
  ["Introduction", 0.000000, 46.149229],
  ["Architectural Knowledge", 46.149229, 104.549320],
  ["Where AI Fits", 104.549320, 147.534127],
  ["Conversation", 147.534127, 203.698481],
  ["A Living Architectural Model", 203.698481, 249.837279],
  ["What Articulate Actually Does", 249.837279, 307.369093],
  ["Architectural Intelligence", 307.369093, 369.231769],
  ["Looking Ahead", 369.231769, 396.845782]
];
...
for (const [title, start, end] of sections) {
  ...
  const weights = chunks.map((chunk) => Math.max(1.5, wordCount(chunk) / 2.55 + 0.28));
...
const srtPath = path.join(subtitleDir, "episode-0002-en.srt");
const transcriptPath = path.join(subtitleDir, "episode-0002-transcript.txt");
fs.writeFileSync(srtPath, cues.map((cue, index) => `${index + 1}\n${srtTime(cue.start)} --> ${srtTime(cue.end)}\n${wrap(cue.text)}\n`).join("\n"));
fs.writeFileSync(transcriptPath, `${[...narrativeSections.values()].join("\n\n")}\n`);

console.log(`Episode 0002 Rough Cut 01 Companion timelines: ${relative(timelineDir)}`);
console.log(`Episode 0002 subtitles: ${relative(srtPath)} (${cues.length} cues)`);
