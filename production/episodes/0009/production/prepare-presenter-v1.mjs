...
const episodeDirectory = path.join(repoRoot, "production/episodes/0009");
...
const rawPath = path.join(here, "raw-transcription/episode-0009-presenter-whisper-base-en.json");
...
const config = {
  ...
  episode: {
    ...
    storyboard: "production/episodes/0009/storyboard.yaml",
    timingMarkers: "production/episodes/0009/production/presenter-v1-markers.json",
    assetRegister: "production/episodes/0009/asset-register.yaml",
    scenePlan: "production/episodes/0009/scene-plan.yaml",
    timeline: "production/episodes/0009/timeline.json"
  },
  presenter: {
    implementation: "continuous-video",
    assetId: "episode-0009-presenter-v1",
  ...
  output: {
    ...
    generatedDirectory: "production/episodes/0009/generated/presenter-v1",
    reviewDirectory: "production/episodes/0009/output/review/presenter-v1",
    narrationAnalysis: "production/episodes/0009/production/presenter-v1-analysis.json"
  },
  scenes
};
...
const storyboard = {
  ...
  episode: {
    ...
    scene_plan: "production/episodes/0009/scene-plan.yaml",
...
const timeline = {
  version: 2,
  generatedFrom: "production/episodes/0009/scene-plan.yaml",
...
const sceneList = {
  episode: {
    ...
    title: plan.episode.title,
    production_status: "presenter-v1",
    timing_authority: "episode-0009-presenter-v1",
...
console.log(`Episode 0009 render config: ${relative(path.join(episodeDirectory, "episode.json"))}`);
console.log(`Episode 0009 publication duration: ${time(selectedDurationSeconds)}`);
...
function buildScene(scene, section, index) {
  ...
  return {
    ...
    assetIds: scene.diagramAssetId
      ? ["A002", scene.diagramAssetId.startsWith("episode-0009-") ? "A005" : (scene.diagramAssetId === "graphrag-traversal" ? "A004" : "A003")]
...
function writePublicationSubtitles(source, duration) {
  ...
  fs.writeFileSync(path.join(subtitleDirectory, "episode-0009-en.srt"), srt);
  fs.writeFileSync(path.join(subtitleDirectory, "episode-0009-transcript.txt"), `${plain}\n`);
  fs.mkdirSync(path.join(episodeDirectory, "output"), { recursive: true });
  fs.writeFileSync(path.join(episodeDirectory, "output/episode-0009-final.srt"), srt);
  fs.writeFileSync(path.join(subtitleDirectory, "episode-0009-subtitle-validation.md"), [
    "# Episode 0009 Subtitle Validation",
    "",
    ...
    "- Architectural terminology checked: Articulate, Architectural Intelligence, Knowledge Model, DCL and Declarative Capability Language",
    ""
  ].join("\n"));
}
