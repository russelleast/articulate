...
const subtitlePath = resolve("production/episodes/0009/output/episode-0009-final.srt");
const thumbnailPath = resolve("production/episodes/0009/publication/thumbnail/episode-0009-thumbnail.png");
const errors = [];
...
const episodeDiagramSources = fs.readdirSync(resolve("production/diagrams/sources/episodes/0009"))
  .filter((name) => name.endsWith(".d2") || name.endsWith(".puml"))
  .map((name) => [`episode-0009-${name.replace(/\.(d2|puml)$/, "")}`, `production/diagrams/sources/episodes/0009/${name}`]);
...
if (fs.existsSync(subtitlePath)) {
  ...
  for (const term of ["Articulate", "Architectural Intelligence", "Knowledge Model", "Declarative Capability Language", "DCL"]) {
    if (!spokenText.includes(term)) errors.push(`Subtitles are missing corrected term: ${term}`);
  }
  for (const rejected of ["architecture intelligence", "architectural building blog", "architecture intent", "AI native"]) {
...
const report = {
  ...
  source: "production/episodes/0009/ep09-narrative.mp4",
  output: config.output.video,
  subtitle: "production/episodes/0009/output/episode-0009-final.srt",
...
if (errors.length) throw new Error(`Episode 0009 validation failed:\n- ${errors.join("\n- ")}`);
console.log(`Episode 0009 validation passed: ${path.relative(repoRoot, reportPath)}`);
