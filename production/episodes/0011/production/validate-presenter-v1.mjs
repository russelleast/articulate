...
const subtitlePath = resolve("production/episodes/0011/output/episode-0011-final.srt");
const thumbnailPath = resolve("production/episodes/0011/publication/thumbnail/episode-0011-thumbnail.png");
const errors = [];
...
const episodeDiagramSources = fs.readdirSync(resolve("production/diagrams/sources/episodes/0011"))
  .filter((name) => name.endsWith(".d2") || name.endsWith(".puml"))
  .map((name) => [`episode-0011-${name.replace(/\.(d2|puml)$/, "")}`, `production/diagrams/sources/episodes/0011/${name}`]);
const diagramAssets = episodeDiagramSources;
...
for (const expected of [
  "episode-0011-memory-overload-state-01",
  "episode-0011-memory-overload-state-02",
  "episode-0011-state-decomposition",
  "episode-0011-context-assembly",
  "episode-0011-context-distinction",
  "episode-0011-state-ownership",
  "episode-0011-retrieval-from-knowledge",
  "episode-0011-state-characteristics",
  "episode-0011-runtime-coordinates",
  "episode-0011-execution-progress"
]) {
  if (!config.scenes.some((scene) => scene.diagramAssetId === expected)) errors.push(`Required visual sequence is missing: ${expected}`);
}
const diagramStates = config.scenes.filter((scene) => scene.diagramAssetId);
for (const scene of diagramStates) {
...
if (config.scenes.length !== 16) errors.push("Episode 11 must contain the approved sixteen-scene argument");
if (!config.scenes.some((scene) => scene.id === "S012" && scene.items.length === 3)) errors.push("Focused cache treatment is missing");

if (fs.existsSync(subtitlePath)) {
  const contents = fs.readFileSync(subtitlePath, "utf8");
  const cues = parseSrt(contents);
  const spokenText = cues.flatMap((cue) => cue.lines).join(" ").replace(/\s+/g, " ");
  ...
  for (const term of ["Articulate", "AI-native", "Knowledge Model", "context", "execution", "durable execution"]) {
    if (!spokenText.includes(term)) errors.push(`Subtitles are missing corrected term: ${term}`);
  }
  for (const rejected of ["Languages and vector indexes", "Ashes belong", "vector stalls", "called it all", "AI native"]) {
...
const report = {
  ...
  source: "production/episodes/0011/ep11-narrator.mp4",
  output: config.output.video,
  subtitle: "production/episodes/0011/output/episode-0011-final.srt",
  thumbnail: "production/episodes/0011/publication/thumbnail/episode-0011-thumbnail.png",
  ...
  checks: [
    ...
    "current visual grammar with complete architectural diagram states",
    "memory decomposition, context assembly and ownership diagrams",
    "directional retrieval derivation and execution-progress treatment",
...
if (errors.length) throw new Error(`Episode 0011 validation failed:\n- ${errors.join("\n- ")}`);
console.log(`Episode 0011 validation passed: ${path.relative(repoRoot, reportPath)}`);
