...
const episodeDirectory = path.join(repoRoot, "production/episodes/0011");
...
const rawPath = path.join(here, "raw-transcription/episode-0011-presenter-whisper-base-en.json");
...
const config = {
  ...
  episode: {
    ...
    storyboard: "production/episodes/0011/storyboard.yaml",
    timingMarkers: "production/episodes/0011/production/presenter-v1-markers.json",
    assetRegister: "production/episodes/0011/asset-register.yaml",
    scenePlan: "production/episodes/0011/scene-plan.yaml",
    timeline: "production/episodes/0011/timeline.json"
  },
  presenter: {
    implementation: "continuous-video",
    assetId: "episode-0011-presenter-v1",
  ...
  output: {
    ...
    height: 1080,
    frameRate: 30,
    video: "production/episodes/0011/output/episode-0011-final.mp4",
    generatedDirectory: "production/episodes/0011/generated/presenter-v1",
    reviewDirectory: "production/episodes/0011/output/review/presenter-v1",
    narrationAnalysis: "production/episodes/0011/production/presenter-v1-analysis.json"
  },
  scenes
};
...
const storyboard = {
  ...
  episode: {
    ...
    scene_plan: "production/episodes/0011/scene-plan.yaml",
...
const timeline = {
  version: 2,
  generatedFrom: "production/episodes/0011/scene-plan.yaml",
...
const sceneList = {
  episode: {
    ...
    title: plan.episode.title,
    production_status: "presenter-v1",
    timing_authority: "episode-0011-presenter-v1",
...
console.log(`Episode 0011 render config: ${relative(path.join(episodeDirectory, "episode.json"))}`);
console.log(`Episode 0011 publication duration: ${time(selectedDurationSeconds)}`);
...
function writePublicationSubtitles(source, duration) {
  ...
  fs.writeFileSync(path.join(subtitleDirectory, "episode-0011-en.srt"), srt);
  fs.writeFileSync(path.join(subtitleDirectory, "episode-0011-transcript.txt"), `${plain}\n`);
  fs.mkdirSync(path.join(episodeDirectory, "output"), { recursive: true });
  fs.writeFileSync(path.join(episodeDirectory, "output/episode-0011-final.srt"), srt);
  fs.writeFileSync(path.join(subtitleDirectory, "episode-0011-subtitle-validation.md"), [
    "# Episode 0011 Subtitle Validation",
    "",
    ...
    "- Architectural terminology checked: Articulate, AI-native, Knowledge Model, context, retrieval, execution and durable execution",
    ""
  ].join("\n"));
}
...
function correctedWords(source) {
  ...
  const phrases = [
    [["languages"], ["Embeddings"]],
    [["retrieve", "documents"], ["retrieved", "documents"]],
    [["ashes"], ["Caches"]],
    [["vector", "stalls"], ["vector", "stores"]],
    [["next", "can", "be", "assembled"], ["Context", "can", "be", "assembled"]],
    [["which", "has", "its", "own", "responsibility"], ["Knowledge", "has", "its", "own", "responsibility"]],
    [["called", "it", "all"], ["called", "a", "tool"]],
...
function correctText(text) {
  return text
    .replace(/\bLanguages and vector indexes\b/g, "Embeddings and vector indexes")
    .replace(/\bretrieve documents\b/gi, "retrieved documents")
    .replace(/\bAshes belong\b/g, "Caches belong")
    .replace(/\bvector stalls\b/gi, "vector stores")
    .replace(/\bNext can be assembled\b/g, "Context can be assembled")
    .replace(/\bWhich has its own responsibility\b/g, "Knowledge has its own responsibility")
    .replace(/\bcalled it all\b/gi, "called a tool")
    ...
    .replace(/\boptimizations\b/gi, "optimisations")
