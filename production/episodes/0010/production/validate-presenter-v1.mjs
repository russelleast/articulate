...
const subtitlePath = resolve("production/episodes/0010/output/episode-0010-final.srt");
const thumbnailPath = resolve("production/episodes/0010/publication/thumbnail/episode-0010-thumbnail.png");
const errors = [];
...
const episodeDiagramSources = fs.readdirSync(resolve("production/diagrams/sources/episodes/0010"))
  .filter((name) => name.endsWith(".d2") || name.endsWith(".puml"))
  .map((name) => [`episode-0010-${name.replace(/\.(d2|puml)$/, "")}`, `production/diagrams/sources/episodes/0010/${name}`]);
...
for (const expected of [
  "episode-0010-conceptual-architecture-to-runtime",
  "episode-0010-runtime-orchestration",
  "episode-0010-runtime-responsibilities-state-01",
  "episode-0010-runtime-responsibilities-state-02",
  "episode-0010-runtime-responsibilities-state-03",
  "episode-0010-memory-responsibilities"
]) {
...
if (config.scenes.length > 15) errors.push("Short transitional episode exceeds the approved fifteen-scene limit");
if (!config.scenes.some((scene) => scene.id === "S005" && scene.items.length === 4)) errors.push("Restrained runtime-candidate evidence scene is missing");
...
if (fs.existsSync(subtitlePath)) {
  ...
  for (const term of ["Articulate", "Architectural Intelligence", "Knowledge Model", "DCL", "Dapr", "Temporal", "LangGraph", "Microsoft Agent Framework"]) {
    if (!spokenText.includes(term)) errors.push(`Subtitles are missing corrected term: ${term}`);
  }
  for (const rejected of ["architecture intelligence", "DCM", "dapper", "land graph", "AI native"]) {
...
const report = {
  ...
  source: "production/episodes/0010/ep10-narrator-edited.mp4",
  output: config.output.video,
  subtitle: "production/episodes/0010/output/episode-0010-final.srt",
  ...
  checks: [
    ...
    "three deliberate runtime-responsibility diagram states",
    "restrained technology-candidate treatment",
...
if (errors.length) throw new Error(`Episode 0010 validation failed:\n- ${errors.join("\n- ")}`);
console.log(`Episode 0010 validation passed: ${path.relative(repoRoot, reportPath)}`);
