...
const framePath = path.join(sourceDirectory, "episode-0010-presenter-540_000.png");
const blendedPath = path.join(sourceDirectory, "episode-0010-presenter-540_000-blended.png");
const editablePath = path.join(sourceDirectory, "episode-0010-thumbnail.svg");
const outputPath = path.join(here, "episode-0010-thumbnail.png");
...
await sharp(outputPath).resize(320, 180).png().toFile(path.join(previewDirectory, "episode-0010-thumbnail-320.png"));
await sharp(outputPath).resize(160, 90).png().toFile(path.join(previewDirectory, "episode-0010-thumbnail-160.png"));
const data = `data:image/png;base64,${(await fs.readFile(outputPath)).toString("base64")}`;
await sharp(Buffer.from(contactSheet(data)), { density: 144 }).resize(1600, 820).png().toFile(path.join(here, "episode-0010-thumbnail-contact-sheet.png"));
console.log(`Episode 0010 thumbnail: ${path.relative(repoRoot, outputPath)}`);
